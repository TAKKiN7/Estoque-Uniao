...
class Adicionar(CTkToplevel):
    ...
    def layout(self):
        ...
        cancelar_B : CTkButton = CTkButton(self, text="Cancelar", font=("itim", 13, "bold"))
    ...
    def confirmar(self, produto : str, modelo: str, quantidade : str):
        if not produto:
            msg.showerror("Erro", "Campo *Nome do Produto* é obrigatório!")
            return
        try:
            qtdd : int = int(quantidade)
        except:
            msg.showerror("Campo *Quantidade em estoque* inválido!")
            return
